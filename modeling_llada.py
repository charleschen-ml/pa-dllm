...
class LLaDAOutput(NamedTuple):
    ...
    """
    A tensor of shape `(batch_size, seq_len)` representing predicted block_size_rel values in [0, 1].
...
class CustomLMOutputWithPast(ModelOutput):
    """
    Custom output class that extends CausalLMOutputWithPast to include block_size_predictions.
    """
...
class LLaDAModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        # Add scheduler head for block_size_rel prediction (optional)
        # Set config.use_scheduler_head = True to enable
        if getattr(config, 'use_scheduler_head', False):
            self.transformer.update(
                {
                    "scheduler_head": nn.Sequential(
                        nn.Linear(config.d_model, config.d_model // 2, bias=config.include_bias, device=config.init_device),
                        nn.ReLU(),
                        nn.Linear(config.d_model // 2, config.d_model // 4, bias=config.include_bias, device=config.init_device),
                        nn.ReLU(),
                        nn.Linear(config.d_model // 4, 1, bias=config.include_bias, device=config.init_device),
                        nn.Sigmoid()  # Output in [0, 1] for block_size_rel
                    )
                }
            )
