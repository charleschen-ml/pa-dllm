...
from scheduler_utils import (
    blocksize_to_class, class_to_blocksize,
    evaluate_classifier, print_metrics_summary,
    plot_confusion_matrix, plot_pr_curve
)


# blocksize_to_class, class_to_blocksize, plot_pr_curve, and plot_confusion_matrix
# are now imported from scheduler_utils
...
def main(use_wandb=True):
    ...
    NUM_CLASSES = 4  # Number of token classes (2 for binary: 1 vs 2 tokens, 5 for 1-5 tokens, etc.)
    ...
    if USE_REGRESSION:
        ...
        # For binary classification, extract AUC metrics for wandb
        if NUM_CLASSES == 2:
            train_pr_auc = train_results.get('pr_auc', 0)
            val_pr_auc = val_results.get('pr_auc', 0)
            test_pr_auc = test_results.get('pr_auc', 0)
            train_roc_auc = train_results.get('roc_auc', 0)
            val_roc_auc = val_results.get('roc_auc', 0)
            test_roc_auc = test_results.get('roc_auc', 0)
    ...
    if not USE_REGRESSION:
        ...
        if NUM_CLASSES == 2 and 'pr_auc' in test_results:
            # Get minority class probabilities for plotting
